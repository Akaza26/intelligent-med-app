...
export async function loginStaff(req, res) {
  try {
    ...
    if (existingOtp) {
      return res
        .status(400)
        .send(new ApiResponse(400, null, "OTP already active."));
    }
    ...
    otp.phone = phone;
    otp.code = code;
    otp.expiry = new Date(Date.now() + 10 * 60 * 1000);

    await otp.save();

    // Send OTP to user

    res.status(200).send(new ApiResponse(200, otp, "OTP sent successfully."));
  } catch (error) {
    console.log(error);
